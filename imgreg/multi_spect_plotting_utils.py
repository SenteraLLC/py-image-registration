...
def normalize_channel(channel):
    """Normalize a channel on range 0-1."""
    return channel / np.max(channel)


def normalize_float_img(img):
    """Normalize each channel on range 0-1."""
...
def show_alignment_result(in_img, out_img, channel_names, dpi=180):
    ...
    for i in range(0, (n_ch - 1) * 2, 2):
        ...
        ax1.imshow(
            in_img[:, :, 0], cmap=plt.cm.spring, alpha=0.75, interpolation="nearest"
        )
        ax1.imshow(
            in_img[:, :, ch_i], cmap=plt.cm.spring, alpha=0.75, interpolation="nearest"
        )
        ax1.title.set_text(
            "Input Image Alignment : "
            + str(channel_names[0])
            + " -->"
            + str(channel_names[ch_i])
        )

        ax2 = plt.subplot(n_ch, 2, i + 2)
        ax2.imshow(
            out_img[:, :, 0], cmap=plt.cm.spring, alpha=0.75, interpolation="nearest"
        )
        ax2.imshow(
            out_img[:, :, ch_i], cmap=plt.cm.spring, alpha=0.75, interpolation="nearest"
        )
        ax2.title.set_text(
...
def show_merged(image, channel_list, blend_ch=-1, image_bounds=None, colors=None):
    ...
    for i in range(3):
        if blend_ch != -1:
            out_img[:, :, i] = (save_img[:, :, blend_ch] * 0.5) + (
                save_img[:, :, channel_list[i]] * 0.5
            )
        else:
